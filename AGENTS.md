...
- `*/requirements.txt`: legacy per-project Python dependency lists.

There is no shared package layer today. Keep new tutorial examples isolated in their own top-level directory with a README, PEP 723 metadata in the runnable script, and a source package.
...
Use the same `uv run --script blogger/agent.py` command in `build-mcp-agent-google-adk` to start the ADK Web UI with the MCP-enabled agent. Dependencies are declared with PEP 723 inline script metadata. The MCP server is launched by the agent when needed.
